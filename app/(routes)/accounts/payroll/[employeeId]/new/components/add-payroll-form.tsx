...
const AddPayrollForm: FC<AddPayrollFormProps> = ({ employee }) => {
  ...
  useEffect(() => {
    if (employee) {
      form.setValue('basicSalary', employee?.basicSalary?.toString() ?? '');
      form.setValue('dataAllowance', employee?.dataAllowance?.toString() ?? '');
      form.setValue(
        'mobileAllowance',
        employee?.mobileAllowance?.toString() ?? ''
      );
      form.setValue(
